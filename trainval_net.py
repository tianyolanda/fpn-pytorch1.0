...
# log and diaplay
  parser.add_argument('--use_tfb', dest='use_tfboard',
                      ...
                      action='store_true')
  parser.add_argument('--use_visdom', dest='use_visdom',
...
if __name__ == '__main__':
  ...
  print('{:d} roidb entries'.format(len(roidb)), logging)
  ...
  if args.resume:
    ...
    print("loading checkpoint %s" % (load_name), logging)
    ...
    print("loaded checkpoint %s" % (load_name), logging)
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      if step % args.disp_interval == 0:
        ...
        if args.use_visdom:
            # visdom
              loss_meter.reset()
              loss_rpn_cls_meter.reset()
              loss_rpn_box_meter.reset()
              loss_rcnn_cls_meter.reset()
              loss_rcnn_box_meter.reset()

              loss_meter.add(loss_temp) 
              loss_rpn_cls_meter.add(loss_rpn_cls)
              loss_rpn_box_meter.add(loss_rpn_box)
              loss_rcnn_cls_meter.add(loss_rcnn_cls)
              loss_rcnn_box_meter.add(loss_rcnn_box)

              vis.plot_many_stack({'train_loss': loss_meter.value()[0],
                                 'rpn_cls_loss': loss_rpn_cls_meter.value()[0],
                                 'rpn_box_loss': loss_rpn_box_meter.value()[0],
                                 'rcnn_cls_loss': loss_rcnn_cls_meter.value()[0],
                                 'rcnn_box_loss': loss_rcnn_box_meter.value()[0]})
    ...
    print('save model: {}'.format(save_name),logging)
